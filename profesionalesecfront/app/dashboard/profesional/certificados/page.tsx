...
export default function ProfesionalCertificatesPage() {
  const { token, loading: authLoading } = useProfesional()
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600">Consulta el estado de emisión y descarga tus certificados emitidos.</p>
      </div>
        ...
        {items.map((item) => (
          <Card key={item.id} className="h-full">
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {item.ponencia_ponente?.nombre_ponente ? (
                    <p className="mt-1 text-sm text-slate-600">Ponente: {item.ponencia_ponente.nombre_ponente}</p>
                  ) : null}
                </div>
            ...
            <CardContent className="space-y-3 text-sm text-slate-600">
              ...
              <p><span className="font-medium text-slate-900">Tipo:</span> {item.scope === "conversatorio" ? "Conversatorio completo" : "Ponencia individual"}</p>
